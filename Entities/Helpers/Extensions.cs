...
public static class Ease {
	...
	public static float InOutExponential(float weight) {
		if (weight < 0.5f) return (float)(Mathf.Pow(2, 20 * weight - 10) / 2);
		return (float)(2 - Mathf.Pow(2, -20 * weight + 10) / 2);
	}
...
public static class DoubleExtension {
	public static float MinMax(this double n, float min, float max) {
		return (float)(1 - ((n - min) / (max - min)));
	}
	public static float RoundToOne(this double n) {
		return (float)(Mathf.Round(n * 10) / 10);
	}
}
