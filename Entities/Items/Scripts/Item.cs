...
public partial class Item : Node
{
    ...
    public override void _Process(double delta)
    {
        if (!Engine.IsEditorHint())
        {
            ...
            if (In3D) ItemUI.Visible = false;
            else ItemUI.Visible = true;
        }
    }
    ...
    public void CheckChildren()
    {
        ...
        {
            if (child is InventoryItemUI) _hasUI = true;
            if (child is InventoryItemUI) _has3D = true;
        }
    ...
    public override string[] _GetConfigurationWarnings()
    {
        ...
        if (!_hasUI)
        {
            warnings.Add("There is no InvetoryItemUI");
        }
        if (!_has3D)
        {
            warnings.Add("There is no IventoryItem3D");
        }
